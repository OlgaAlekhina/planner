...
class GroupPermission(permissions.BasePermission):
    """
    разрешает пользователю удалять, редактировать и добавлять участников только в свою собственную
    группу
    """
...
class EventPermission(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        # этого события
        if view.action in ['retrieve']:
            return request.user.is_authenticated and (obj.author == request.user or request.user in
                                                      [groupuser.user for groupuser in obj.users.all()])
